...
export default function DropboxIntegration({ companyId, darkMode = false, onDocumentImported }: DropboxIntegrationProps) {
  ...
  const checkConnection = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/integrations/dropbox?companyId=${companyId}&action=status`);
      const data = await res.json();
      setConnected(data.connected);
      setEmail(data.email);
    } catch (error) { console.error("Failed to check Dropbox connection:", error); }
    setLoading(false);
  };

  const connectDropbox = () => { window.location.href = `/api/auth/dropbox?companyId=${companyId}`; };

  const disconnectDropbox = async () => {
    try {
      await fetch(`/api/integrations/dropbox?companyId=${companyId}`, { method: "DELETE" });
      setConnected(false);
      setEmail(null);
      setFiles([]);
  ...
  const loadFiles = async () => {
    ...
    try {
      const res = await fetch(`/api/integrations/dropbox?companyId=${companyId}&action=list`);
      const data = await res.json();
  ...
  const importFile = async (file: DropboxFile) => {
    setImporting(file.id);
    try {
      const res = await fetch("/api/integrations/dropbox", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ companyId, filePath: file.path, fileName: file.name }),
      });
      const data = await res.json();
      if (data.ok) {
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${connected ? "bg-blue-100" : darkMode ? "bg-gray-600" : "bg-gray-200"}`}>
              <svg className={`w-6 h-6 ${connected ? "text-blue-600" : darkMode ? "text-gray-400" : "text-gray-500"}`} viewBox="0 0 24 24" fill="currentColor">
                <path d="M6 2L0 6l6 4-6 4 6 4 6-4-6-4 6-4-6-4zm12 0l-6 4 6 4-6 4 6 4 6-4-6-4 6-4-6-4zM6 14l6 4 6-4-6-4-6 4z"/>
              </svg>
            </div>
            <div>
              <h4 className={`font-medium ${darkMode ? "text-white" : "text-gray-900"}`}>Dropbox</h4>
              {connected ? (
                <p className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Connected as {email}</p>
              ) : (
                <p className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Import documents from your Dropbox</p>
              )}
            </div>
          </div>
          {connected ? (
            <div className="flex items-center gap-2">
              ...
              <button onClick={disconnectDropbox} className={`p-2 rounded-lg ${darkMode ? "text-gray-400 hover:text-red-400" : "text-gray-500 hover:text-red-500"}`} title="Disconnect">
                <Unlink className="w-5 h-5" />
              </button>
            </div>
          ) : (
            <button onClick={connectDropbox} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700">
      ...
      {showFilePicker && files.length === 0 && (
        <div className={`${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} rounded-lg border p-8 text-center`}>
          ...
          <p className={`text-sm mt-1 ${darkMode ? "text-gray-500" : "text-gray-400"}`}>Upload PDFs, Word docs, or spreadsheets to your Dropbox</p>
        </div>
      )}
    </div>
  );
