...
export default function UpgradeBanner({ companyId, plan, email }: Props) {
  ...
  return (
    <div className="rounded-xl border border-[#0060F0]/20 bg-gradient-to-r from-[#0060F0]/5 to-amber-50 p-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Zap className="h-5 w-5 text-[#0060F0]" />
            <h3 className="text-base font-semibold text-gray-900">Upgrade to Sidekick Pro</h3>
            <span className="text-sm font-bold text-[#0060F0]">$200/mo</span>
          </div>
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center gap-2 px-5 py-2.5 bg-[#0060F0] text-white rounded-lg text-sm font-semibold hover:bg-[#0052CC] disabled:opacity-50 transition-colors flex-shrink-0"
        >
          {loading ? (
