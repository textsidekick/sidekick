...
const ALL_MESSAGES: Msg[] = [
  ...
  {
    id: 1,
    text: "Logged. Conveyor 3 — likely bearing wear. Priority: HIGH. Work order #4521 created, assigned to Mike T. Parts: 6205-2RS bearing (2 in stock).",
    sender: "sidekick",
  },
  { id: 2, text: "Where's the 6205 bearing?", sender: "user" },
  {
    id: 3,
    text: "6205-2RS bearing is in Parts Cage B, Shelf 3. 2 in stock. Need help with anything else?",
    sender: "sidekick",
  },
  { id: 4, isVoice: true, sender: "user" },
  {
    id: 5,
    text: "Second shift starts at 3:00 PM. Shift lead today is Carlos R. You're in Warehouse B.",
    sender: "sidekick",
  },
  { id: 6, text: "¿Dónde recojo mi uniforme?", sender: "user" },
  {
    id: 7,
    text: "Recoge tu uniforme en Recursos Humanos, Edificio A, sala 102. Abierto de 7 AM a 4 PM.",
    sender: "sidekick",
  },
  { id: 8, text: "Wet floor near dock 2, slip hazard", sender: "user" },
  {
    id: 9,
    text: "Safety hazard flagged. Priority: CRITICAL. Alert sent to shift supervisor. Cleanup work order #4522 created for immediate response.",
    sender: "sidekick",
  },
];
...
export default function SidekickSmsDemo() {
      ...
      <div ref={messagesRef} className="sk-chat-messages flex-1 overflow-y-auto px-3.5 py-3.5 flex flex-col gap-1.5">
        ...
        {messages.map((msg, i) => (
          <div
            ...
            style={{ animation: "sk-msg-in 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275) both" }}
          >
            {msg.isVoice ? (
              <div className="flex items-center gap-2 px-3 py-2 bg-[#007AFF] rounded-[18px_18px_4px_18px] min-w-[170px]">
                <div className="w-[22px] h-[22px] rounded-full bg-white/95 flex items-center justify-center">
                  <svg width="9" height="11" viewBox="0 0 10 12" fill="#007AFF">
              ...
              <div
                className={`max-w-[240px] px-[13px] py-[9px] text-sm leading-[1.4] ${
                  msg.sender === "user"
                    ? "bg-[#007AFF] text-white rounded-[18px_18px_4px_18px]"
                    : "bg-[#E9E9EB] text-black rounded-[18px_18px_18px_4px]"
                }`}
              >
                {msg.text}
              </div>
              ...
              {[0, 0.2, 0.4].map((delay, i) => (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: "50%",
                    background: "#8E8E93",
                    animation: `sk-typing-bounce 1.4s ease-in-out ${delay}s infinite`,
                  }}
                />
              ))}
            </div>
      ...
      <div className="px-3 pt-2.5 pb-3 border-t border-[#E5E5EA]/50 bg-[#F9F9F9] flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          {recording ? (
            ...
            <div
              className={`flex-1 bg-white rounded-[18px] px-[13px] py-2 text-[13px] border border-[#E5E5EA] min-h-[30px] ${
                isTyping ? "text-black" : "text-[#8E8E93] flex items-center"
              }`}
            >
              {isTyping ? (
                <span>
                  {typingText}
                  <span style={{ borderRight: "2px solid #007AFF", animation: "sk-blink 1s step-end infinite" }}>
                    &zwnj;
                  </span>
                </span>
              ) : (
